...
func parse(s string) (nd Node, err error) {
	if s == "" {
	...
	nd, pos, err := parseBytes(bytes, 0, size)
	if err != nil {
		return
	}
	if pos < size-1 {
		err = errors.New("Found extra text after root")
		return
	}
...
func parseBytes(bytes []byte, idx int, size int) (nd Node, pos int, err error) {
	var (
		state int
		start int
		//attr  Attribute
	...
	for i < size {
		...
		case 0: //start
			if c == '<' {
				...
				err = errors.New("Found extra text before root")
				return
			}
		case 1: //inside node name
			if c == '>' {
				if i == start {
					err = errors.New("No node name")
					return
				}
				nd.Name = string(bytes[start : i-1])
				state = 5
			} else if isWS(c) {
				if i == start {
					err = errors.New("No node name")
				...
				i, err = getExpected('>', bytes, i+1, size)
				if err != nil {
					return
				}
				nd.Name = string(bytes[start:end])
				state = 6
			}
		case 2: //inside node
			if c == '/' {
		...
		case 3: //inside attibute name
			...
			state = 4
			//attr = Attribute{
			//	Name: string(bytes[start:end]),
			//}
		case 4: //inside attribute value

		case 5: //end node start
...
func getExpected(expected byte, bytes []byte, idx int, size int) (int, error) {
	var space bool
	...
	for i < size {
		...
		if c == expected {
			...
			return i, fmt.Errorf("Expected %s, found %s", string(expected), string(c))
		}
		i++
	}

	return i, fmt.Errorf("Expected %s, found nothing", string(expected))
}
